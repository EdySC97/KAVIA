...
# 3) Conexión a la base de datos (recurso compartido)
@st.cache_resource
def conectar_db():
...
def add_item(orden_id, producto_id, cantidad, precio):
    ...
    cur.execute("""
      ...
      VALUES (%s, %s, %s, %s, %s);
    """, (orden_id, producto_id, cantidad, precio, subtotal))
    conn.commit()
...
# 7.1) Selección de categoría y producto
...
# 7.2) Mostrar detalle, total y opciones de finalizar/imprimir
