...
# 2. Conexión a Postgres (recurso compartido)
@st.cache_resource
def conectar_db():
...
def get_or_create_order(mesa_id, personas):
    ...
    if not df.empty:
        ...
        if existing != personas:
            cur = conn.cursor()
            cur.execute("UPDATE ordenes SET personas=%s WHERE id=%s",
                        (personas, oid))
            conn.commit()
        return oid
    cur = conn.cursor()
...
def generar_ticket_pdf(mesa, personas, orden_id, items, total):
    ...
    for r in items.itertuples():
        pdf.cell(0,6,
                 f"{r.cantidad:>2} x {r.producto:<20} $ {r.subtotal:>6.2f}",
                 ln=True)
    pdf.cell(0,6, "-"*40, ln=True)
    pdf.cell(0,6, f"TOTAL: $ {total:.2f}", ln=True, align="R")
    return pdf.output(dest="S").encode("latin1")

# 5. Sidebar: carrito de mesas abiertas
...
# 6. Área principal
# 6.1 Selección de mesa y personas
mesas_df = get_tables()
if mesas_df.empty:
...
# 6.2 Selección de categoría y producto (ahora incluye Bebidas)
...
if items.empty:
    ...
    if st.button("💳 Finalizar y generar ticket PDF"):
        pdf_buf = generar_ticket_pdf(mesa_sel, personas, orden_id, items, total)
        st.download_button(
